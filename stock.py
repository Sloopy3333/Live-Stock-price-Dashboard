...
gainer_data.append(go.Bar(x=list(gainers_df.Symbol), y=list(gainers_df["% Change"])))
...
loosers_data.append(go.Bar(x=list(loosers_df.Symbol), y=list(loosers_df["% Change"])))
...
def graph_genrator(n_clicks, ticker, chart_name):
    ...
    if n_clicks >= 1:  # Checking for user to click submit button
        ...
        if chart_name == "SMA":
            close_ma_10 = talib.SMA(df.close, 10)
            close_ma_15 = talib.SMA(df.close, 15)
            close_ma_30 = talib.SMA(df.close, 30)
            close_ma_100 = talib.SMA(df.close, 100)
            fig = go.Figure(
                data=[
                    go.Scatter(
        ...
        if chart_name == "EMA":
            close_ema_10 = talib.EMA(df.close, 10)
            close_ema_15 = talib.EMA(df.close, 15)
            close_ema_30 = talib.EMA(df.close, 30)
            close_ema_100 = talib.EMA(df.close, 100)
            fig = go.Figure(
                data=[
                    go.Scatter(
        ...
        if chart_name == "RSI":
            ...
            fig = go.Figure(
                data=[go.Scatter(x=list(df.index), y=list(rsi), name="RSI")],
                layout={
                    "height": 900,
                    "title": chart_name,
                    "showlegend": True,
            ...
            go.Indicator(
                ...
                gauge={
                    ...
                    "steps": [
                        ...
                        {"range": [prev_close - 100, prev_high], "color": "green"},
                    ],
                    "threshold": {
